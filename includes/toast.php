<?php
if (isset($_SESSION['toast'])): ?>
    <div class="toast-container">
        <div class="toast" id="toast">
            <?php if ($_SESSION['toast']['type'] === 'error'): ?>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M508.5-291.5Q520-303 520-320t-11.5-28.5Q497-360 480-360t-28.5 11.5Q440-337 440-320t11.5 28.5Q463-280 480-280t28.5-11.5ZM440-440h80v-240h-80v240Zm40 360q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/></svg>
            <?php elseif ($_SESSION['toast']['type'] === 'success'): ?>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="m424-296 282-282-56-56-226 226-114-114-56 56 170 170Zm56 216q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/></svg>
            <?php endif; ?>
            <?= htmlspecialchars($_SESSION['toast']['message']); ?>
        </div>
    </div>

    <script>
        const fade_duration = 500;
        const display_duration = 3000;
        const toastEl = document.getElementById('toast');
        
        setTimeout(() => toastEl.classList.add('open'), 10);
        setTimeout(() => toastEl.classList.remove('open'), display_duration);
        setTimeout(() => toastEl.parentElement.remove(), display_duration + fade_duration);
    </script>

    <?php unset($_SESSION['toast']); ?>
<?php endif; ?>

